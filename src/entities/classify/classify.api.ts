...
export const classifyAIBookmarksMutation = async ({
  ...
  const prompt = `${JSON.stringify(bookmarkCache)}에 있는 링크들을 ${JSON.stringify(categories)} 배열의 각 category 별로 링크들을 분류해줘. 이때 리턴 값은 { id, category: 카테고리명, bookmark: [{ id, title, url }] 형태의 배열값으로 줘. 그리고 category의 값이 중복되어서는 안돼. 그리고 id값은 '1','2','3'은 제외해줘. 부가적인 설명은 필요하지 않아.`;
