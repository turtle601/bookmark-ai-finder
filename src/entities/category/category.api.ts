...
export const categoryAIQuery = async (): Promise<ICategory[]> => {
  ...
  const prompt = `${JSON.stringify(bookmarkCache)} 데이터를 기반, 어떠한 폴더를 기준으로 북마크를 분류해야할지 추천해줘. 리턴 값은 { id: 추천하는 폴더명, text: 추천하는 폴더명 }의 배열값으로 줘. 부가적인 설명을 필요하지 않아.`;
  const response = await createGeminiRequest({ prompt });

  return mapAICategorList(response);
};
