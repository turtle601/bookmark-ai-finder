...
export const categoryAIQuery = async (): Promise<ICategory[]> => {
  ...
  const prompt = `${JSON.stringify(bookmarkCache)} 데이터를 기반, 어떠한 폴더를 기준으로 북마크를 분류해야할지 추천해줘. 리턴 값은 { id: 추천하는 폴더명, text: 추천하는 폴더명 }의 배열값으로 줘. 이때, 중복된 폴더값과 북마크바와 기타 북마크, 모바일 북마크 폴더는 제외 시켜줘. 부가적인 설명은 필요하지 않아.`;
